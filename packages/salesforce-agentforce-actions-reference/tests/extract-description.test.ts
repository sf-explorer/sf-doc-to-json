...
describe('Description Extraction', () => {
    ...
    test('should handle case where h1 and p are siblings directly', () => {
        const html = `
            <main>
                <h1>Create Incident Resolution Summary</h1>
                <p>Bypass the Welcome Message When Handing Off Ongoing Conversations to...</p>
                <p>This action creates a summary of incident resolution.</p>
            </main>
        `;
        
        const description = extractDescription(html, 'Create Incident Resolution Summary');
        
        // Should find the second paragraph, not the bypass one
        expect(description).toBe('This action creates a summary of incident resolution.');
        expect(description).not.toContain('Bypass');
    });
});
